...
class LatentSurvivalGenerator(FeedbackAwareGenerator):
    ...
    def generate(self, sampling_parameters: dict[str, Any]) -> list[Image.Image]:
        ...
        width = sampling_parameters["width"]
        height = sampling_parameters["height"]
        ...
        # Determine actual number of images to generate this round
        if self.is_first_batch:
            num_new_images = self.batch_size
        else:
            num_new_images = self.batch_size - 1
        ...
        output = self.diffusion_pipeline(
            prompt=self.prompt, 
            num_images_per_prompt=num_new_images,
            latents=new_latents,
            **sampling_parameters)
